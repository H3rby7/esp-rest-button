...
esp_err_t restart_handler(httpd_req_t *req)
{
  ...
  while (secondsToReboot > 0)
  {
    ESP_LOGW(WEB_AP_CONF_TAG, "Rebooting in %d seconds", secondsToReboot);
    vTaskDelay( 1000 / portTICK_PERIOD_MS );
    secondsToReboot--;
  }
  esp_restart();
...
httpd_handle_t start_webserver(void)
{
    ...
    ESP_LOGI(WEB_AP_CONF_TAG, "Starting server on port: '%d'", config.server_port);
    if (httpd_start(&server, &config) == ESP_OK) {
        // Set URI handlers
        ESP_LOGI(WEB_AP_CONF_TAG, "Registering URI handlers");
    ...
    ESP_LOGI(WEB_AP_CONF_TAG, "Error starting server!");
    return NULL;
}